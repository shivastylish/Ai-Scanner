...
from ai_screener.market_data.normalization.bybit_normalizer import BybitNormalizer
...
def test_normalize_maps_raw_ohlcv_into_the_standard_schema() -> None:
    ...
    result = BybitNormalizer().normalize(raw, "BTCUSDT")

    assert list(result.columns) == STANDARD_COLUMNS
    row = result.iloc[0]
    assert row["provider"] == "bybit"
